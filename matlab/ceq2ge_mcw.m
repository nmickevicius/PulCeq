function ceq2ge_mcw(ceq, sysGE, ofname, varargin)
...
toppe.writeentryfile('toppe0.entry', ...
    'filePath', '/usr/g/bin/research/toppe/', ...
...
toppe.preflightcheck('toppe0.entry', 'seqstamp.txt', sysGE);

%% Put TOPPE files in a .tar file (for convenience)
if toppeVersion > 5
    system(sprintf('tar cf %s toppe0.entry seqstamp.txt modules.txt scanloop.txt cores.txt', ofname));
else
    system(sprintf('tar cf %s toppe0.entry seqstamp.txt modules.txt scanloop.txt', ofname));
end
for p = 1:ceq.nParentBlocks
...
if ~arg.verbose
    if toppeVersion > 5
        system('rm toppe0.entry seqstamp.txt modules.txt scanloop.txt cores.txt');
    else
        system('rm toppe0.entry seqstamp.txt modules.txt scanloop.txt');
    end
    for p = 1:ceq.nParentBlocks
